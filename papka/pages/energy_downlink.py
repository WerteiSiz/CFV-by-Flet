...
from papka.components.earth_visualization import EarthVisualization
...
def show_energy_dowlink(container, page, inputs  ,buttons_row1,EarthVisualization):
    """Отображение формы энергетического расчета с визуализацией Земли"""
    ...
    container.controls.append(
        ft.Container(
            content=ft.Text(
                "Расчет параметров линии связи вниз от спутника к земной станции с визуализацией атмосферных слоев",
                size=16,
                text_align=ft.TextAlign.LEFT,
            ),
            alignment=ft.alignment.top_left,
    ...
    # Создаем визуализацию Земли (инициальное состояние без параметров)
    earth_viz = EarthVisualization()

    # Сохраняем ссылку на визуализацию для последующего обновления
    container.earth_visualization = earth_viz

    # Двухпанельная компоновка: Визуализация слева, Форма справа
    layout = ft.ResponsiveRow(
        [
            # Левая панель: Визуализация Земли
            ft.Container(
                content=ft.Card(
...
def show_results_energy_dowlink(container, page, inputs  ,buttons_row1):
    try:
        ...
        length = 0.3 / chastota  # Длина волны в метрах

        # ОБНОВЛЯЕМ ВИЗУАЛИЗАЦИЮ ЗЕМЛИ с реальными параметрами
        if hasattr(container, 'earth_visualization'):
            container.earth_visualization.update_values(
                wavelength=length,
                frequency=chastota,
                atmosphere_loss=atmos,
    ...
    container.controls.append(footer)

    # Добавляем новый результат и сохраняем ссылку на него
